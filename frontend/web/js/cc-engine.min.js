(function(a, b) {
    ...
    b.compose = function() {
        ...
        return function() {
            for (var b = arguments,
            d = a.length - 1; d >= 0; d--) b = [a[d].apply(this, b)];
            return b[0]
        }
    };
    ...
    b.pick = function(a) {
        var c = {};
        j(b.flatten(i.call(arguments, 1)),
        function(b) {
            b in a && (c[b] = a[b])
        });
        return c
    ...
    b.clone = function(a) {
        return ! b.isObject(a) ? a: b.isArray(a) ? a.slice() : b.extend({},
        a)
    };
    b.tap = function(a, b) {
        b(a);
    ...
    b.template = function(a, c, d) {
        ...
        function(a, b) {
            return "'+\n_.escape(" + w(b) + ")+\n'"
        }).replace(d.interpolate || u,
        function(a, b) {
            return "'+\n(" + w(b) + ")+\n'"
        }).replace(d.evaluate || u,
        function(a, b) {
            return "';\n" + w(b) + "\n;__p+='"
        }) + "';\n";
    ...
    function(a) {
        ...
        m.prototype[a] = function() {
            var d = this._wrapped;
            b.apply(d, arguments);
            var e = d.length; (a == "shift" || a == "splice") && e === 0 && delete d[0];
...
$(function() {
    if (typeof window.__olab_custom === "function") {
        window.__olab_custom()
    }
    if (/Apple.*Mobile.*Safari/.test(navigator.userAgent)) {
        var $multipleSelect = $("select[multiple]");
        if ($multipleSelect.length) {
            $multipleSelect.each(function() {
                var $select = $(this);
                var $phantom_ios_noOpt = $('<option value="" disabled selected>' + ($select.attr("noSelectionLabel") || "") + "</option>");
                $select.append($phantom_ios_noOpt).change(function() {
                    var selected = $select.val();
                    if ($phantom_ios_noOpt.attr("selected")) {
                        $phantom_ios_noOpt.attr("selected", null)
                    } else {
                        if (!selected || selected.length === 0) {
                            $phantom_ios_noOpt.attr("selected", "selected")
                        }
                    }
                })
            })
    ...
    $('<div id="submitResultAlert" role="alert" aria-live="assertive"></div>').insertBefore($("#thankyou"));
    $("#cdu .cdu_radios fieldset label").each(function() {
        var $this = $(this),
        val = +$this.prev().val(),
        reader_text = ["Very Dissatisfied", "Dissatisfied", "Neutral", "Satisfied", "Very Satisfied"][ - 1 + val],
        reader_symbols = {
            "Very Dissatisfied": "- -",
            Dissatisfied: "-",
            Neutral: "+ -",
        ...
        if (reader_text && reader_symbols) {
            $this.html(reader_symbols + '<span class="screen_reader">&nbsp;' + reader_text + "</span>")
        }
    });
    var $ts = $("#topic_selector select"),
    $transformedContainer = $("#custom_questions"),
    $cqContainers = $("#untransformed > table, #q_untransformed table#questionsontop"),
    label = "<label for='q<%= index %>'><%= labelText %></label>",
    selectTemp = _.template(label + "<select id='q<%= index %>' class='<%= reqQuestion %>' name='<%= name %>'><option value=''></option><option value='<%= value1 %>'><%= value1 %></option><option value='<%= value2 %>'><%= value2 %></option></select><div class='clear'></div>"),
    radioTemp = _.template("<div class='<%= className %>'><input id='<%= id %>' class='<%=other%>' name='<%= name %>' type='radio' value='<%= i %>'/><label for='<%= id %>'><%= label %></label><span class='position_<%= index %>'></span></div>");
    if ($ts.length) {
        var $defaultTSel = $ts.children().first();
        $("#ts_label").prepend($defaultTSel.text());
        $defaultTSel.text("");
        $ts.attr("id", "topic_select");
        $ts.attr("aria-required", "true");
        setTimeout(function() {
            $("#topic_selector select").append(" ")
        },
        1e3)
    }
    $cqContainers.each(function(i) {
        var $cq = $(this),
        $noTransform = $(".dontTransform", $cq);
        if ($noTransform.length) {
            $cq.parents(".q_container").append($noTransform);
            return
        }
        var $radios = $("input[type=radio]", $cq),
        $select = $("select", $cq),
        $textarea = $("textarea", $cq),
        $checkboxes = $("input[type=checkbox]", $cq),
        $section = $("<div/>").addClass("section"),
        name = $("input, textarea, select", $cq).first().attr("name"),
        label = $(".questionSpan", $cq).text(),
        $label,
        reqQuestion = !!$(".questionSpan.reqQuestion", $cq).length,
        reqQMarkup = '&nbsp;<span class="req_question">*</span><span class="screen_reader">Required</span>',
        radioLabels = ["&#8722; &#8722;", "&#8722;", "&#43; &#8722;", "&#43;", "&#43; &#43;"],
        $clear = $("<div/>").addClass("clear");
        var $fieldset, $labels, symbolLabels, rSize, $legend, nps, $descriptions, rLabel;
        if ($radios.length) {
            $fieldset = $("<fieldset />").addClass("nps nps_" + $radios.length).toggleClass("req_question", reqQuestion);
            $labels = $("label, .answer", $cq).filter(function() {
                return $(this).text().replace(/\s/g, "")
            });
            symbolLabels = $labels.text().indexOf("+") !== -1;
            rSize = $radios.length;
            $legend = $("<legend/>").html("<span class='width_fix'>" + label + "</span>");
            nps = rSize === 11;
            $descriptions = $(".description", $cq);
            if (reqQuestion) {
                $legend.find("span").append(reqQMarkup);
            ...
            function(j) {
                if (symbolLabels) {
                    rLabel = radioLabels[j]
                } else if ($labels.length) {
                    rLabel = $labels.eq(j).text()
                } else {
                    rLabel = nps ? j: j + 1
                }
                $fieldset.append(radioTemp({
                    name: name,
                    className: j === rSize - 1 ? "last": "not_last",
                    id: name + "_" + j,
                    label: rLabel,
                    index: j,
                    other: $radios[j].className.indexOf("other") !== -1 ? "other": "",
                    i: $radios.length === 11 ? j: $radios.length > 2 ? j + 1 : rLabel
                }))
            });
            if ($descriptions.length) {
                $section.addClass("descriptors");
                $descriptions.each(function() {
                    var $this = $(this),
                    $el = $fieldset.find(".position_" + (Number($this.data("position")) - 1));
                    $el.html($this.text().replace(/\s/g, "&nbsp;"))
                })
            }
            if ($radios.closest(".other").length) {
                $fieldset.append('<input class="explanation" style="display: none" type="text" placeholder="Please explain"/>')
            }
            $section.append($fieldset);
            $section.append($clear.clone())
        } else if ($textarea.length === 1) {
            $label = $("<label/>").attr("for", name).text(label);
            if (reqQuestion) $label.append(reqQMarkup);
            $section.append($label, $("<textarea/>").attr({
                name: name,
                id: name
            }).toggleClass("req_question", reqQuestion), $clear.clone())
        } else if ($select.length === 1) {
            var $other = $("<div/>").addClass("none");
            $label = $("<label/>").attr("for", name).text(label);
            if (reqQuestion) $label.append(reqQMarkup);
            $other.append($("<label/>").attr({
                "for": name + "_oth",
                "class": "screen_reader"
            }).text(label), $("<input/>").attr({
                type: "text",
                name: name,
                id: name + "_oth"
            }));
            $section.append($label, $select.removeAttr("onchange").attr("id", name).toggleClass("req_question", reqQuestion), $clear.clone(), $other, $clear.clone())
        } else if ($checkboxes.length) {
            $fieldset = $("<fieldset />").addClass("checkboxes").toggleClass("req_question", reqQuestion);
            labels = $(".answerSpan", $cq).html().replace(/<\/input>/i, "").split(/<br>/i);
            rSize = $checkboxes.length;
            $legend = $("<legend/>").html("<span class='width_fix'>" + label + "</span>");
            if (reqQuestion) {
                $legend.find("span").append(reqQMarkup);
                $fieldset.attr("aria-required", "true")
            ...
            $fieldset.append($legend);
            _(rSize).times(function(i) {
                var index = labels[i].search(/[^>]+$/),
                uID = _.uniqueId("checkbox");
                $fieldset.append($checkboxes.eq(i).attr("id", uID), $("<label/>").text(labels[i].substr(index)).attr("for", uID), $clear.clone())
            });
            $section.append($fieldset);
            $section.append($clear.clone())
        } else {
            $label = $("<label/>").attr("for", name).text(label);
            if (reqQuestion) $label.append(reqQMarkup);
            $section.append($label, $("input[type=text]", $cq).removeAttr("class").toggleClass("req_question", reqQuestion), $("<div/>").addClass("clear"))
        }
        $cq.parents(".q_container").append($section);
        $radios.add($textarea).add($("input[type=text]", $cq)).remove();
        $radios = $textarea = null
    })
});
$(function() {
    _.extend(CC, {
    ...
    if (location.search.indexOf("simtouch=1") !== -1) {
        Modernizr.touch = true;
        CC.$html.removeClass("no-touch").addClass("touch")
    }
    Modernizr.addTest({
        "position-fixed": function() {
            ...
            test.prepend(fixed, normal);
            CC.$body.prepend(test);
            result = normal.offset().top === 0;
            test.remove();
            return result
        }
    });
    function setLayout() {
        ...
        twoColumn = windowWidth > BoxWidths.two_column;
        if (twoColumn) {
            twoColumn = navigator.userAgent.search("MSIE 10.0") > -1 || navigator.userAgent.search("MSIE 9.0") > -1 || navigator.userAgent.search("MSIE 8.0") > -1 || navigator.userAgent.search("MSIE 7.0") > -1 || typeof IE6 !== "undefined" ? windowWidth >= document.body.scrollWidth: windowWidth >= $(document).width()
        }
        document.body.className = twoColumn ? "two_column": "one_column";
        CC.$body.trigger("layoutset", [twoColumn])
    }
    if (Modernizr.framed) {
        $(document.body).addClass("two_column")
    } else {
        setLayout();
        $(window).on("resize", _.debounce(setLayout, 300))
    }
    var $banner = $("#banner");
    if (!$banner.find("img").length && $banner.height() < 38) $banner.height(38);
    if (typeof IE6 !== "undefined") {
        $("input[type=text]").addClass("text");
        if ($("#no_comments").length) {
            $("#cdu legend").each(function() {
                t = $(this);
                t.after("<span class='legendie6'>" + t.text() + "</span>")
            })
        }
    }
    setTimeout(function() {
...
$(function() {
    if (!Modernizr.boxsizing) {
        var inputs = $('#custom_questions textarea, #custom_questions input[type="text"]'),
        $commentBox = $("#comment_box"),
        noCDU = $("#no_cdu #comment_box");
        fakeBorderBox(null, CC.$body.hasClass("two_column"));
        CC.$body.on("layoutset", fakeBorderBox)
    }
    function fakeBorderBox(e, twoColumn) {
        if (twoColumn || Modernizr.framed) {
            inputs.width(BoxWidths.two_column - 42);
            if (noCDU.length) {
                noCDU.width(BoxWidths.two_column - 42)
            } else {
                $commentBox.width(BoxWidths.right_column - 42)
            }
            return
        }
        inputs.width(function(index, width) {
            return $(this).parent().width() - 22
        })
    }
});
$(function() {
    var bHeight = CC.$body.height();
    if (Modernizr.framed) {
        if (Modernizr.postmessage) parent.postMessage(bHeight, "*");
        else parent.location = location.search.split("=")[1] + "#OL=" + bHeight
    } else if (CC.RACK_ENV !== "development" || location.search.indexOf("resize=1") !== -1) {
        var tallerThanWindow = bHeight > screen.height,
        popHeight = tallerThanWindow ? screen.height - 80 : bHeight + 50,
        popupWidth = 545 + 25;
        window.resizeTo(popupWidth, popHeight);
        window.moveTo(screen.width / 2 - popupWidth / 2, screen.height / 2 - popHeight / 2)
    }
}); (function($) {
    var blank;
    ...
    $.fn.placeholder = function() {
        ...
        controlFieldFocus = navigator.userAgent.search(/[ ](Kindle|Silk)/) > -1 ?
        function(e) {
            var $placeholder = placeholders[e.target.id];
            if ($placeholder) {
                if (e.type === "blur") {
                    $placeholder.toggleClass("visible", blank(e.target))
                } else if (e.type === "focus") {
                    $placeholder.toggleClass("visible", false)
                }
            }
...
$(function() {
    if (Modernizr.boxsizing) $("#comment_box").placeholder();
    if (location.search.indexOf("no-validation=1") !== -1 && CC.RACK_ENV !== "production") return;
    var $validate = $("#nps_container fieldset.nps, #overall_control, #topic_selector.req_question select, #qtop fieldset.req_question, #qtop select.req_question, #qtop input.req_question, #qtop textarea.req_question"),
    $win = $(window),
    ...
    validators = {
        ...
        fieldset: function($el) {
            return ! $("input[type=radio]", $el).serializeArray().length
        },
        input: function($el) {
            return $el.val() === ""
        }
    };
    $validate = $validate.add("input.req_question, textarea.req_question, select.req_question, fieldset.req_question", "#custom_questions");
    validators.textarea = validators.input;
    $("form").on("submit", addErrors).on("change", "#nps_container fieldset.nps, .req_question, #overall_control, #topic_selector select",
    function(e) {
        var disabled = true;
        if (validate().length === 0) disabled = false;
        $buttons.toggleClass("disabled", disabled)
    });
    function addErrors(e) {
        $(".error").removeClass("error");
        var fieldsWErrors = validate();
        if (fieldsWErrors.length) {
            e.preventDefault();
            e.stopImmediatePropagation();
            fieldsWErrors.addClass("error");
            fieldsWErrors.find("input, select, textarea, fieldset").attr("aria-invalid", "true");
            $formErrors.show();
            $win.scrollTop(0)
        }
    }
    function validate() {
        ...
        $validate.each(function() {
            var $el = $(this),
            nName = this.nodeName.toLowerCase();
            if (validators[nName]($el)) fieldsWErrors = fieldsWErrors.add($el.closest(".section"))
        });
        return fieldsWErrors
    }
    $("#custom_questions").on("click", "fieldset.req_question input[type=radio]",
    function(e) {
        $(e.target).closest("fieldset").change()
    })
});
$(function() {
    ...
    function convertNpsToOverallForSubmit(npsValue) {
        if (typeof npsValue === "string" && npsValue.toLowerCase() === "zero") return 1;
        else if (npsValue <= 6) return 1;
        else if (npsValue <= 8) return 3;
        else return 5
    }
    $("form").on("submit",
    function(e) {
        e.preventDefault();
        CC.$html.removeClass("sticky_header");
        var $form = $(e.target),
        npsValue = $("#nps_container input[type=radio]:checked").val(),
        overallWCAG = $("#overall_control"),
        npsSubmitValue;
        if (npsValue) {
            if (overallWCAG.length === 0) {
                npsSubmitValue = convertNpsToOverallForSubmit(npsValue)
            }
            $form.append($('<input name="overall" type="hidden"/>').val(npsSubmitValue))
        }
        ajaxOptions = {
            type: "POST",
        ...
        $.ajax(ajaxOptions).done(function(msg) {
            var $uccTy = $("#thankyou"),
            $overallControl = $("#overall_control"),
            raterVal = "";
            if (npsValue) {
                npsMapping = {
                    Zero: 1,
                ...
                raterVal = npsMapping[npsValue] - 1;
                raterVal = raterVal.toString()
            }
            raterVal5_0 = $("#overall").find(".rater_selected").find(".rater_button_value").text();
            if (raterVal5_0) {
                raterVal = raterVal5_0
            }
            raterVal5_1 = $("input[name=overall]:checked").length > 0 ? $("input[name=overall]:checked").val() - 1 : "";
            if (raterVal5_1.toString()) {
                raterVal = raterVal5_1.toString()
            }
            if (msg.indexOf("<body") !== -1) {
                var $olTy = $(msg.split(/<body.*?>/gi)[1].split("</body>")[0]),
                $integrations = $(".integration", $olTy),
                $tyElements = $("#tyHeader, #tyMessage", $olTy),
                $isLinks = $(".isLink", $olTy),
                $tyIs = $("#is_links", $uccTy);
                ...
                $olTy.each(function() {
                    var $this = $(this);
                    if ($this[0].className == "integration") {
                        integrationHTML = integrationHTML + $this[0].outerHTML
                    }
                });
                var intiframe = document.createElement("iframe");
                var content = "<!DOCTYPE html>" + "<head><title>Opinionlab Integrations</title>" + "<body>" + integrationHTML + "</body></html>";
                document.body.appendChild(intiframe);
                intiframe.contentWindow.contents = content;
                intiframe.src = 'javascript:window["contents"]';
                intiframe.width = "0";
                intiframe.height = "0";
                intiframe.tabindex = "-1";
                $tyElements.each(function() {
                    var $this = $(this);
                    if (!isBlank($this.text())) $("#" + $this.attr("id"), $uccTy).html($this.html())
                });
                $tyIs.append($isLinks)
            }
            if (raterVal) {
                $(".rater_button", $uccTy).addClass("order_" + raterVal)
            } else {
                $(".rater_button").remove()
            }
            CC.$html.removeClass("fail").addClass("thankyou");
            $("#thankyou").css("display", "block");
            setTimeout(function() {
                $("#submitResultAlert").append($("#thankyou"))
            },
            500)
        }).fail(function(msg) {
            CC.$html.removeClass("sticky_header");
            CC.$html.addClass("fail");
            setTimeout(function() {
                $("#submitResultAlert").append($("#fail"))
            },
            500)
        })
    })
...
$(function() {
    if (typeof IE6 !== "undefined") return;
    var $focussed;
    CC.$body.on("focusin", "input, textarea, select, .slider_track, #rater a",
    function(e) {
        var $t = $(e.target);
        if ($focussed && $focussed.find("input[type=radio]").length) $focussed.find(".selected").removeClass("selected");
        if ($focussed && $t.parents("section").hasClass("selected")) return;
        else if ($focussed) $focussed.removeClass("selected").find("selected").removeClass("selected");
        if ($t.is("input[type=radio]")) $t.parent("div").addClass("selected");
        $focussed = $t.parents(".section").addClass("selected")
    });
    CC.$body.on("change", "input[type=radio]",
    function(e) {
        $(e.target).focus()
    });
    if (CC.$body.hasClass("two_column") && !$("#qtop").length) {
        try {
    ...
    function normalizeEvent(e) {
        if (e.originalEvent && "touches" in e.originalEvent) {
            var previousPreventDefault = e.preventDefault;
            if (e.originalEvent.touches.length > 0) {
                e = e.originalEvent.touches[0]
            } else {
                e = e.originalEvent
            }
            e.preventDefault = previousPreventDefault
        }
        return e
    }
    $.fn.slider = function(options) {
        var context = options.context || window;
        var scopedOnMove = function(e) {
            options.onMove.call(context, normalizeEvent(e))
        };
        var bindToMove = function(e) {
            if (e.which !== null && e.which != 3) {
                e.preventDefault();
                options.onDown.call(context, normalizeEvent(e));
                $("html").on(CC.Events.move, scopedOnMove).on(CC.Events.up, unbindToMove)
            }
        };
        var unbindToMove = function(e) {
            $("html").off(CC.Events.move, scopedOnMove).off(CC.Events.up, unbindToMove);
            if ("onUp" in options) options.onUp.call(context, normalizeEvent(e))
        };
        return this.on(CC.Events.down, bindToMove)
    }
})(jQuery);
var Slider = function(el) {
    this.$el = $(el);
    this.$track = $(".slider_track", this.$el);
    this.$handle = $(".slider_handle", this.$track);
    this.$glyphs = $(".slider_glyph", this.$el);
    this.$radios = $("input", this.$el);
    ...
    this.$track.slider({
        onDown: function(e) {
            $("*:focus").blur();
            $(".slider_track").removeClass("focus");
            this.$track.addClass("focus dragging");
            this.$track.trigger("focusin");
            this.deselect().render(e)
        },
        onMove: this.render,
        onUp: function(e) {
            this.render(e).select();
            this.$track.removeClass("dragging")
        },
        context: this
    });
    $(window).on("layoutset", _.bind(function() {
        this.measure();
        if (this.positionProperty != "left") {
            this.moveHandle(this.positions[this.nearestIndex])
        }
    },
    this));
    this.moveHandle(50);
    this.$select.on("change", $.proxy(function(e) {
        this.nearestIndex = Number(e.target.value) - 1;
        this.moveHandle(this.positions[this.nearestIndex], "animate");
        this.deselect().select();
        this.$track.addClass("focus");
        this.setHandleClass();
        this.focused = e.target
    },
    this));
    this.$select.on("focus", $.proxy(function(e) {
        var slider = this;
        $(document.body).on("keydown",
        function(e) {
            if (e.which === 39) {
                if (slider.nearestIndex != 4) slider.nearestIndex++;
                slider.moveHandle(slider.positions[slider.nearestIndex], "animate")
            }
            if (e.which === 37) {
                if (slider.nearestIndex !== 0) slider.nearestIndex--;
                slider.moveHandle(slider.positions[slider.nearestIndex], "animate")
            }
            slider.deselect().select();
            slider.$track.addClass("focus");
            slider.setHandleClass()
        })
    },
    this));
    this.$select.on("blur", $.proxy(function(e) {
        $(document.body).off("keydown");
        this.$track.removeClass("focus")
    },
    this));
    return this
};
Slider.prototype = {
    ...
    nearestIndex: 2,
    eventX: 0,
    positionProperty: Modernizr.prefixed("transform") || "left",
    measure: function() {
        this.offsetLeft = this.$track.offset().left;
        this.trackWidth = this.$glyphs.width() * 5;
        var paddingPercentage = (this.padding + this.handleWidth / 2) / this.trackWidth * 100;
        this.minLeft = paddingPercentage;
        this.maxLeft = 100 - paddingPercentage;
        return this
    },
    getXPosition: function() {
        return this.eventX - this.offsetLeft
    },
    render: function(e) {
        this.eventX = !e.pageX ? this.eventX: e.pageX;
        this.$handle.stop();
        var newLeft = this.getXPosition() / this.trackWidth * 100;
        if (newLeft < this.minLeft) newLeft = this.minLeft;
        if (newLeft > this.maxLeft) newLeft = this.maxLeft;
        this.moveHandle(newLeft);
        this.setNearestGlyphIndex();
        this.setHandleClass();
        return this
    },
    select: function() {
        this.moveHandle(this.positions[this.nearestIndex], "animate");
        this.$select.val([this.nearestIndex + 1]);
        var fromHandle = this.$track.hasClass("dragging");
        if (fromHandle) _.defer(_.bind(function() {
            this.$select.trigger("focus")
        },
        this));
        $(this.$glyphs.get(this.nearestIndex)).addClass("slider_glyph_selected");
        return this
    },
    deselect: function() {
        this.$glyphs.removeClass("slider_glyph_selected");
        return this
    },
    moveHandle: function(left, animate) {
        if (this.positionProperty == "left") {
            this.$handle[animate ? "animate": "css"]({
                left: left + "%"
            })
        } else {
            left *= this.trackWidth / 100;
            this.$handle.get(0).style[this.positionProperty] = "translate(" + Math.round(left) + "px, 0px)"
        }
    },
    setNearestGlyphIndex: function() {
        var nearestGlyphIndex = Math.floor(this.getXPosition() / this.trackWidth * this.positions.length);
        if (nearestGlyphIndex < 0) nearestGlyphIndex = 0;
        if (nearestGlyphIndex > 4) nearestGlyphIndex = 4;
        this.nearestIndex = nearestGlyphIndex;
        return nearestGlyphIndex
    },
    setHandleClass: function() {
        var handle = this.$handle.get(0),
        classNames = handle.className;
        if (classNames.indexOf(this.nearestIndex + 1) == -1) {
            handle.className = classNames.replace(/\d/, this.nearestIndex + 1)
        }
    }
};
$(function() {
...
$(function() {
    ...
    var $raterButtons = $("#rater_buttons"),
    $raterButtonAnchors = $(".rater_button"),
    $select = $("#overall select"),
    width = 0,
    offsetLeft = 0,
    selectedIndex,
    down = false;
    function measure() {
        offsetLeft = $raterButtons.offset().left;
        width = $raterButtons.width()
    }
    measure();
    $(window).on("layoutset", measure);
    function selectByEvent(e) {
        var i = Number(e.target.id.split("_")[1]);
        selectedIndex = i;
        e.preventDefault();
        select(e.target, i)
    }
    function select(button, i) {
        var $button = $(button),
        $prevSelected = $button.siblings(".rater_selected").removeClass("rater_selected");
        if (typeof IE6 !== "undefined") {
            button.id += "_selected";
            if ($prevSelected.length) $prevSelected[0].id = $prevSelected[0].id.replace("_selected", "")
        }
        $button.addClass("rater_selected");
        $select.val(i + 1).trigger("change").trigger("focusin")
    }
    $raterButtons.on("click",
    function(e) {
        e.preventDefault()
    }).keydown(function(e) {
        if (e.which === 13) selectByEvent(e)
    }).slider({
        onDown: function(e) {
            down = true;
            selectByEvent(e)
        ...
        onMove: function(e) {
            e.preventDefault();
            var x = (e.pageX || 0) - offsetLeft;
            if (x < 0) x = 0;
            if (x > width) x = width;
            var i = Math.floor(x / width * 5);
            if (i != selectedIndex) {
                selectedIndex = i;
                select($raterButtonAnchors[i], i)
            }
        },
        onUp: function() {
            down = false
    ...
    if (!Modernizr.touch) {
        $raterButtonAnchors.hover(function() {
            if (!down) $(this).addClass("hover")
        },
        function() {
...
$(function() {
    if (!Modernizr["position-fixed"]) return;
    var $win = $(window),
    margin;
    $win.on("scroll",
    function() {
        if (CC.$html.hasClass("thankyou")) return;
        if (CC.$body.hasClass("two_column")) margin = 0;
        else margin = 20;
        CC.$html.addClass("sticky_header");
        $("#main").css("padding-top", $("#banner").height() + 10 + margin + "px");
        $win.off("scroll")
    });
    if (Modernizr.touch) {
        CC.$body.on("focusin", "input[type=text], textarea",
        function(e) {
            CC.$html.addClass("hide_banner")
        }).on(CC.Events.down, _.debounce(function(e) {
            if ($(e.target).is("input[type=text], textarea")) return;
            CC.$html.removeClass("hide_banner")
        },
        2e3))
    }
}); (function($) {
    $.fn.counter = function() {
        return this.each(function() {
            max_length = parseInt($(this).attr("data-max-length"), 10);
...
$(function() {
    if (!Modernizr.framed) return;
    $("a.sr_close").on("click",
    function(e) {
        e.preventDefault();
        if (Modernizr.postmessage) parent.postMessage("submitted", "*");
        else parent.location = location.search.split("=")[1] + "#OL=submitted"
    })
});