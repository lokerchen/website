...
<?php $this->beginPage() ?>
<?php
$company_name = \common\models\Config::getConfig('company_name');
$company_add = \common\models\Config::getConfig('address');
$company_city = \common\models\Config::getConfig('city');
$company_postcode = \common\models\Config::getConfig('postcode');
...
<head>
  ...
  var PAYMENT_INFO = {Minpay:"<?=$this->context->getConfig('Minpay')?>",
  Minimum:"<?=$this->context->getConfig('Minimum')?>",
  Opentime:"<?=@strtotime($this->context->getConfig('Open_time'))?>",
  Closetime:"<?=@strtotime($this->context->getConfig('Close_time'))?>",
  Nowtime:"<?=@strtotime(date('H:i'))?>",
  NeedInfo:"You need to spend <?=$this->context->getConfig('currency').$this->context->getConfig('Minimum')?> or<br/>more to order for delivery",
  ...
  <div class="container">
    ...
    <div class="container">
      ...
      if(isset($holiday['options'])&&is_array($holiday['options'])&&$holiday['status']):
        ...
        if($holiday_flag){
          $holiday_message = !empty($holiday_message) ? $holiday_message : \Yii::t('info','Shop is now on holiday, please order again later! <p>Thank you.</p>');
      ...
      <div class="modal fade pro-order-modal" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
        <div class="modal-dialog candialog ">
          <div class="modal-content cancal-content" style="padding: 15px; width: 550px;">
            ...
            <div class="modal-header mymodal-head">
              ...
              <h2 class="open-time">
                ...
                if($holiday_flag){
                  ...
                  // return $this->redirect(['/site/product']);
                  echo $holiday_message;
                } else { echo 'Shop opening at '.$this->context->getConfig('Open_time'); }
                ?>
              </h2>
              <h2 class="close-time" style="display:none;">Shop closed at <?=$this->context->getConfig('Close_time')?></h2>
            </div>
            <div class="excontent">


              <? if ($holiday_flag){ echo ' '; } else { echo '<p>Minimum spend for delivery '.$this->context->getConfig('currency').$this->context->getConfig('Minimum').'</p>'; }?>
            </div>
            ...
            <div><button class="checkout-button confirm" type="button" data-dismiss="modal" aria-label="Close"><? if($holiday_flag){$holiday_message = !empty($holiday_message) ? $holiday_message : \Yii::t('info','Confirm'); echo 'Confirm'; } else { echo 'Pre-order for later'; }?></button> </div>
          </div>
        </div>
      </div>
      ...
      <div class="modal fade allergy-modal" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
        <div class="modal-dialog modal-sm candialog " style="width:550px;">
          <div class="modal-content cancal-content" style="padding: 15px; width: 550px;">
            ...
            <div>
              <?php echo isset($this->context->allergy['content']) ? showContent($this->context->allergy['content']) : '';?>
            </div>
      ...
      if( \Yii::$app->getSession()->hasFlash('start_cookies') ) {
        echo '<script>modal_alert(\''.\Yii::$app->getSession()->getFlash('start_cookies').'\',"accept");</script>';
      }
      ?>
